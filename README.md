...
Smart playlists are playlists whose contents are generated by a saved filter (for example, "all unwatched Action movies added this year"). Hestia has full Smart Playlist support: filters and playlist contents both copy across accounts and across servers, and a smart playlist can be converted into a normal (static) playlist as part of the copy if you want to detach the destination from the source filter.

The Playlist Transfer flow supports four directions:

- **Filter copy, user-to-user on the same server.** Duplicate a smart playlist's filter onto another managed user so they get their own personalized copy that keeps updating against their own library state.
- **Filter copy, server-to-server.** Carry the saved filter from Server A onto a managed user on Server B. A destination-vocabulary preflight runs before the write, so any tag or field missing on the destination is surfaced before the playlist is created, not after.
- **Content copy** (same server or cross-server). Resolve the smart playlist's current contents on the source and write them as a normal playlist on the destination. The destination does not track future changes to the source filter; this is the "freeze what the smart playlist looks like today" mode.
- **Smart-to-static conversion.** Convert the source smart playlist into a normal playlist (preserving the items at conversion time) as part of the transfer, then deliver it to the destination user or server.

Where a specific filter still cannot be translated (server-specific id with no mapping, destination vocabulary the preflight could not satisfy), Hestia logs the row under "Smart Playlist - Requires Manual Recreation" and saves the original filter URL in the run log for reference. Translatable filters in the same job still land normally.
...
| **Skip playlists** | Omits the playlist gather entirely. Watch history, collections, and ratings transfer normally. | Migrating to a fresh server and you would rather rebuild playlists by hand, or you want a faster snapshot and plan to handle playlists in a separate Playlist Transfer job afterwards. | You have playlists (smart or static) you want preserved at the destination as part of this run. |
...
- Want to handle playlists separately via a dedicated Playlist Transfer job: enable **Skip playlists** for the snapshot, then run the Playlist Transfer flow afterwards.
