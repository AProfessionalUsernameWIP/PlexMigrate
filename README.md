...
* **Docker + Web UI** (added in v0.8.0): `docker compose up --build` (or `make docker`). Brings up a FastAPI backend and a React frontend in two containers. The web dashboard shows the same live data as the terminal one. Every CLI flag has a form control. You can save scheduled recurring exports. Jump to [Docker and Web UI](#docker-and-web-ui).

**New in PR-13: snapshot rename + capture pipeline + on-demand JSON.**
The "export -> `.plexexport.json` file" model is now three layers:
`media.db` (live working store), per-server snapshot `.db` files
(immutable point-in-time copies), and `snapshots.db` (registry
indexing the files). The Exports panel reads the registry, grouping
snapshots by server with per-server retention enforcement and a
db_admin-gated "Clear all snapshots for this server" button.
Settings has a new Snapshot Retention block (global ceiling +
per-server overrides; global wins when an override is higher).
...
**New in v0.9.7  refinements and data fidelity.** The network charts now propagate data across the full 60-second window even under high traffic, the Y-axis only grows (never collapses on quiet periods), and elapsed time freezes at the final duration when a job ends instead of climbing through the post-finish retain window. The Thread Pool panel now populates during imports too  it was blank for every import path including direct-transfer-import because of an oversight that had been there since v0.5.0. Plex Pass personal collections are now correctly captured per-user and restored per-user; pre-v0.9.7 they were silently dropped from every snapshot. The Logs tab gained a case-insensitive keyword filter with match highlighting, and the import file picker shows readable short-form labels like `Movies  My Server (2026-05-11 10:32)` instead of bare filenames.

**New in v0.12.1  engine integration for the SQLite data layer + Hard Stop.** Direct transfers (both the in-memory and chained-fallback paths) now feed source-side payloads into `media.db` after each library is gathered. The new `ingest_snapshot_payload` helper enforces a strict dedup discipline by rating_key: a library-level collection or playlist visible to every home user lands exactly once under `user_handle=""`, and only genuinely-personal user variants get their own rows under `user_handle=<username>`  fixing the future bug where naive per-user iteration would have written each server-wide collection N times. The resolver gains **Tier 0**  a DB-cached ratingKey lookup that runs before the existing GUID/path/title tiers  so items the cache has seen before resolve in a single `fetchItem(rating_key)` call instead of paying Plex's slow `getByGuid` round-trip on every GUID. The cache populates as you run direct transfers, so each subsequent run against the same source server gets faster than the last. A v2 schema migration adds the `server_items(item_id, server_id, rating_key)` join table the cache rides on. Finally, the dashboard has a new **Hard Stop (force)** button beneath the regular Stop that tears down the shared HTTP session so a hung run collapses in seconds rather than minutes  confirmation-gated because any in-flight items get recorded as failures.

**New in v0.12.0  Networking tab + SQLite data layer foundation.** A dedicated **Networking** tab now lives next to Dashboard / Run Job / Servers, showing live HTTP health for every registered server in one place  server name, status dot, ping latency, requests/sec, average response time, status-code histograms (60-second window + cumulative-since-boot), and a rate-limit feed for 429s and 503s. It works in every UI state: when idle (driven by the 30 s ping poll), during a single-destination job, and during a fan-out (one card per destination). The Dashboard tab's inline network charts stay unchanged for single-destination runs  the new tab is an addition, not a replacement.

Under the hood, telemetry moved from the per-job `DashboardState` to a process-lifetime server-keyed collector (`server/network_collector.py`) keyed by URL host. The engine's HTTP response hook reports into it, the ping poller reports into it, and the WebSocket payload carries a new `servers_network` array. The same refactor unblocks the fan-out gap where Network details previously vanished when the panel switched to multi-card layout.

Also in v0.12.0  the **SQLite data layer** foundation lands as `server/media_db.py` and `services/guid_translator.py`. A new `media.db` (in `server_data/`) carries six tables  `items` (keyed by upstream metadata IDs like `imdb://tt0133093`, not Plex `ratingKey`), `watch_events`, `ratings`, `playlists`, `collections`, `servers`  with migration scaffolding (`schema_version` table + version-numbered DDL) and a `GET /api/db/stats` endpoint surfacing schema version, row counts, on-disk size, and last-updated timestamps. The GUID translator collapses legacy Plex agent strings (`com.plexapp.agents.thetvdb://121361?lang=en`) to canonical form (`tvdb://121361`) so cross-server matching is efficient. **Engine integration is staged for v0.12.1**  snapshotter DB-primary writes, importer DB-first reads, and resolver Tier-0 lookups are large changes to long-tested engine code that deserve their own release.

**New in v0.11.0  opt-in web-UI authentication.** PlexMigrate now ships a JWT-based login system you can turn on with a single environment variable (`PLEXMIGRATE_AUTH_ENABLED=true`). When off (the default), the app is byte-for-byte the same as v0.10.x  no login screen, no UI change. When on, the first browser visit walks you through creating an admin account, then every API call and the live WebSocket require a token from `/api/auth/login`. Tokens are 24-hour-lived. By default they're held in `sessionStorage` so reloading the tab keeps you signed in but closing it signs you out; tick **Remember me on this device** on the login form to escalate to `localStorage` (persists across browser restarts up to the JWT's 24-hour expiry). The signing key (`server_data/.auth_secret`) and the operator-account database (`server_data/auth.db`) auto-generate on first boot. Override the key with `PLEXMIGRATE_AUTH_SECRET` if you manage your own secrets.

**Also in v0.10.0  Servers tab hardening.** The Add Server form's Test Connection button no longer registers the server as a side effect (pre-v0.10.0 it always added on success). Every successful probe captures the connected Plex server's own friendly name and `machineIdentifier` (a stable install ID), so the result banner shows you which physical Plex you actually reached  a Plex account's token works on every server that account owns, so a "connection OK" doesn't by itself prove anything about which server you hit. Saving a new server now refuses if the probed `machineIdentifier` is already registered under a different name, and the UI flags the collision before you click Save. Each row also gets a one-click **Refresh** button that re-probes the saved credentials and updates the cached library counts and names.

**New in v0.10.0  fan-out transfer.** One job, many destinations. The Run Job form's destination picker is now a multi-select; pick two or more servers and the job becomes a fan-out  it reads from the source once and writes the same data into every destination **in parallel**, each on its own thread with its own dashboard card, log directory, and error tracking. A destination that fails (network blip, auth revoked, server offline) doesn't take its siblings down. The dashboard auto-switches between the classic single-destination view and the fan-out view based on what's running. Single-destination jobs are unchanged  pick one server, get the same flow you've always had. Under the hood the entire run-scoped engine state  the dashboard, the active Plex URL/token, the per-library accumulator dicts  was converted from plain module globals to `contextvars.ContextVar` so each destination thread sees its own isolated values and engine workers spawned inside a destination inherit them automatically. Jump to [Fan-Out Transfer](#fan-out-transfer).
