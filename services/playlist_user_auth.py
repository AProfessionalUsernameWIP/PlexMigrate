"""User resolution + per-user authentication for Playlist Management.

Extracted from :mod:`services.playlist_copy`: the user-identification
and per-user-token machinery is reusable on its own. The snapshotter,
the dev console, and the jobs layer all resolve users and build
``UserContext`` objects through this surface, and the upcoming
play-count / ratings features need the same per-user auth chain.

What lives here:

  * :func:`_find_user` - resolve a ``UserSpec`` from a flexible
    identifier (managed_users cache, then live adapter, then the
    identity-map walk).
  * :func:`_user_context_for` - build a ``UserContext``, running the
    Plex Home per-user-token chain (saved token -> PIN -> admin
    fallback) for non-owner Plex users.
  * Supporting helpers: owner detection, the managed_users lookups,
    the app_user_uuid resolver, and the same-logical-user comparison.

``services.playlist_copy`` re-exports the names other modules import,
so existing ``from services.playlist_copy import _find_user`` (etc.)
...
def _user_context_for(
    ...
    * strict=False -> silently fall back to the admin/owner context
    ...
    # Owner detection. A gate that only checked ``role == 'owner'`` is
    # not enough: when the resolved ``user_spec`` has ``is_admin=True``
    # but a non-owner role (cached row drift, identity-map walk landing
    # on a managed-flavor row, etc.) the per-user auth chain runs and
    # the create_playlist call gets routed through a per-user
    # PlexServer with the WRONG token - the write succeeds on that
    # user's view but never shows up under the owner.
    #
...
def _managed_users_row(
    ...
    """
    # Route through services.user_activity_filter.list_active_users so
