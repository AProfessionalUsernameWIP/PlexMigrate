...
Owns ``restore_watch_history`` and the Plex HTTP write helpers it relies on
(``_scrobble``, ``_set_resume_position``), plus the pure Merge-mode view-count
math (``_compute_merge_views_to_add``). Extracted from the legacy
``services.restorer`` god-file so the per-kind restore code is browsable on
its own; the public ``services.restorer`` facade re-exports every symbol the
rest of the codebase and the test suite import.
"""
...
def _facade_scrobble(base_url: str, rating_key: int, token: str) -> None:
    """
    Dispatch to ``services.restorer._scrobble`` so existing test
            ...
            if mode == "replace":
                if current_view_count > stored_view_count:
                    try:
                        ...
                        # Replace promises an exact point-in-time
                        # mirror. The destination viewCount is HIGHER
                        # than the snapshot and we could not reset it,
                        # so the scrobble path below cannot reach the
                        # stored target - it would leave the count too
                        # high and then record that wrong end state as
                        # a success. Record a failure and skip this
                        # item instead of silently mis-reporting it.
                        logger.warning(
