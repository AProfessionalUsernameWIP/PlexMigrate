...
Single chokepoint every backend-touching code path uses to ask "give
me the users I should be working with on this server." Replaces the
inconsistent mix of:

  * media_db.list_managed_users(include_hidden=False)
  * adapter.list_users(ctx) raw enumeration
  * services.auth.get_home_users(...) for Plex
  * _load_tombstoned_dest_usernames() ad-hoc set-builder
  * various other call sites that bypass filtering entirely

with a consistent layered gate:

  1. Tombstones (per-server + global) — already supported today via
     media_db's hidden_scope filter.
  2. Auth health — drops users whose consecutive_auth_failures > 0
     when the engine-wide ``user_activity_filter_enabled`` tunable
     is on (default OFF). The signal columns the gate reads live on
     managed_users; Phase B of Plan[ACTIVE-USER-FILTER] ships them.

Phase A (this module's first form) ships the three-function API and
migrates every call site to use it. record_auth_result is a no-op
until Phase B adds the schema columns; should_auto_tombstone always
returns False until Phase C wires the sweeper. The point of doing
Phase A first is to land the chokepoint with ZERO observable change
so future phases just flip switches against an already-consistent
surface.

Public API:

  list_active_users(server_id, *,
                    include_owner=True,
                    bypass_health_filter=False) -> List[Dict]
  record_auth_result(*, server_id, username,
                     result, detail=None) -> None
  should_auto_tombstone(*, server_id, username) -> bool

Every backend-touching caller in services/ and server/ should use
list_active_users. Admin / UI surfaces that explicitly want to see
inactive users pass bypass_health_filter=True; that's the only
...
log = logging.getLogger("plexmigrate.services.user_activity_filter")


# Result enum normalised across backends. probe_user() and
# record_auth_result() both speak in these four values.
RESULT_OK = "ok"
    ...
    *,
    ...
    for r in rows:
        ...
        if failures is None or int(failures) <= 0:
            kept.append(r)
            continue
        # Failures > 0: drop. The sweeper / capture sites have flagged
        # this user as not-currently-working.
        log.debug(
