"""
User-activity sweeper audit log.

Dedicated logger for the user-activity sweeper. Same shape as
``services.sync_log``:

  * Namespace ``plexmigrate.user_activity``
  * ``propagate=False`` so records never reach the ``plexmigrate``
    root logger (and therefore never land in a running job's
    runtime.log).
  * RotatingFileHandler at ``<data_dir>/user_activity.log`` with the
    same rotation knobs as the other audit logs.
  * Token-scrubber install on the handler.

The LogsPanel surfaces this file under Servers > Logs > User
Activity Sweep (Plan[ACTIVE-USER-FILTER] §10.5) by registering a
``"user-activity"`` category on the /api/application-logs route.
"""
...
def get_user_activity_logger() -> logging.Logger:
    """Return the dedicated user-activity sweeper logger.
    Idempotent. propagate=False; writes to user_activity.log."""
