"""Background playlist-cache refresher.

Operator feedback: "loading playlists can take a bit; we need to just
read directly from the database, and we should be having a separate
thread that checks in with each server it has a connection to for
each user that it claims a playlist for, to validate the objects of
each playlist."

Implementation: a single daemon thread loops every
``playlist_cache_background_refresh_interval_seconds`` (default 15
min, tunable [60s, 86400s]) and re-fetches every (server, user) pair
currently in ``playlist_cache_db`` via the existing
``services.playlist_copy.refresh_user_cache`` helper. Failures per
pair are caught + logged so one bad server can't poison the loop.

The thread starts at app boot from ``server/app.py``'s startup hook
and stops on process exit (daemon=True means it doesn't block
shutdown). The enabled tunable
(``playlist_cache_background_refresh_enabled``) is checked on every
iteration so the operator can flip it off live without a restart."""
...
log = logging.getLogger("plexmigrate.services.playlist_cache_refresher")
...
def _job_queue_busy() -> bool:
    ...
    contaminate the active job's run log + dashboard (cache
    refresher was hitting unrelated
    servers + users mid-batch and the log lines looked like the
    batch was doing work it wasn't)."""
    try:
...
def _refresh_loop() -> None:
    ...
    while not _STOP_EVENT.is_set():
        ...
        # Pause while jobs are
        # running. The refresher's connections + list_playlists
        # calls were leaking into batch-job run logs + dashboards,
        # making it look like the batch was hitting unrelated
        # servers + users.
        if _job_queue_busy():
            log.debug(
