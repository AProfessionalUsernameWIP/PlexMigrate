...
Mirrors the layout of ``services.restore.plex_native.watch``: the ``_rate_item``
...
against ``services.restore.plex_native._rate_item`` still win.
"""
...
def _facade_rate_item(
    ...
    """
    Dispatch to ``services.restore.plex_native._rate_item`` so existing test
