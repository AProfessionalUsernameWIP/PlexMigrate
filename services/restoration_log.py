"""
Per-run restoration log writer.

Phase 2 of the dashboard / log reorganisation. The restore engine
emits one line per (item, user, metric) outcome into
``<run_log_dir>/restoration.log`` so an end user reviewing a finished
restore can answer:

* Was this item actually restored, or did the destination already
  match the snapshot and the engine did nothing?
* If something failed, what was the resolver / API reason?
* How many smart playlists were skipped, and which ones?
* How long did each metric take?

The writer is intentionally separate from the existing
``_record_success`` / ``_record_failure`` aggregates (which drive the
dashboard counters). Those describe the resolver outcome; this writer
describes the *end user-visible action taken*. A resolver succeeds and
the engine still does nothing is a NOOP here, not a success - this is
the distinction the end user asked for ("don't call things fully
restored if they already existed").

Layout of ``restoration.log``:

* One line per (item, user, metric) tuple. Status is one of
  RESTORED / NOOP / SKIPPED / FAILED.
* A trailing ``Restoration summary`` block produced by
  :meth:`RestorationLogWriter.close_with_summary` with totals
  per-status, per-metric, per-library, per-user, plus wall-clock.

The writer is best-effort: any exception inside swallows quietly so
the restore itself is never aborted by a diagnostic failure.
"""
...
# Field width for the status column so per-line alignment stays
# readable in a plain-text viewer. Pad to len("RESTORED") = 8.
_STATUS_WIDTH = 8
_METRIC_WIDTH = 16


_log = logging.getLogger("plexmigrate.services.restoration_log")
...
class RestorationLogWriter:
    ...
    def __init__(
        ...
        *,
        ...
        self._closed = False
        # USER-MGMT-IDENTITY-AUDIT cosmetic follow-on: when supplied,
        # every user reference written or aggregated goes through
        # services.user_display.display_for_logging(dest_server_id, user).
        ...
        # Counters that the summary block reads. Defaulting via
        # defaultdict keeps the inner code branch-free.
    ...
    def _emit(
        ...
        *,
        ...
        try:
            ...
            # Diagnostic only: never let a logging failure abort a
            # restore. The dashboard counters track real progress; this
            # log is parallel.
    ...
    def _render_summary(self, *, wall_seconds: float) -> list:
        ...
        # Per-library breakdown - report change-count + failure-count
        # only (NOOP / SKIPPED don't surface here; they'd add noise the
        # end user already sees in the per-line entries).
        ...
        # Per-user breakdown - same shape as per-library: changes-only.
        if self._per_user:
