...
//     renders only when the user's actual picker selection
...
//
// The tab strip is the single backend selector, and the sub-card's
  ...
  return (
    <div className="panel" style={{ padding: '10px 14px' }}>
      ...
      <nav className="tabs sub-tabs" role="tablist" aria-label="Workflow backend selector">
        {/* FECORE-10: hoist backendCounts out of the .map() so it runs once per render, not 3x */}
        {(() => {
          const counts = backendCounts(servers);
          return (['plex', 'jellyfin', 'emby'] as BackendType[]).map((b) => {
          const c = counts[b];
          const disabled = c === 0;
      ...
      {/* Per-tab banner. Plex: production-ready, no banner.
          Jellyfin / Emby: first-run informational notice.
          Cross-platform tab: open selection mode - source / dest
          pickers below show every registered server. Cross-backend
          routes (source.backend != dest.backend) are gated
          frontend-side; a same-backend route from the cross-platform
          tab is permitted. */}
      {workflowMode === 'cross' && (
        <div
          className="banner"
        ...
        >
          ...
          to be configured but Submit is gated until cross-backend
          support ships.
        </div>
      )}
      {showFirstRunNotice && (
        ...
        >
          ...
          enabled. Adapter contract tests are green
      ...
      {/* Cross-backend sub-card: D-RATE radio + D-OWNER user-create
          ...
          user's actual picker selection produces a cross-backend
          route (sourceBackend !== destBackend), which is only
      ...
      {isCrossBackend && (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 12 }}>
            ...
            This cross-backend route surfaces extra controls below.
          </div>
          ...
          {/* User creation on the destination is handled per-row inside
